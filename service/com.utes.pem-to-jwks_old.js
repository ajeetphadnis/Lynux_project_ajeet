 ...
 */
	...
	async function pem2jwks (uid, filjwt, pass, req, res, next) {
		...
		let rawdata = fs.readFileSync(filjwt, 'utf8');
		keystore.add(rawdata, 'pem').then(function(_) {
		    const jwks = keystore.toJSON(DUMP_PRIVATE_KEY);
		    var jwt = JSON.stringify(jwks, null, 4);
		    global.jwstr = jwt;
		    console.log("pem2jwks001:      " + jwt);
		    fs.writeFile("./pem2jwtks.json", jwt, function(err) {
				if(err) {
...
pem2jwks ('karan123456',  './user_certs/'+'karan123456'+'_selfsigned.crt', 'password', '','','');
