...
const session = require('express-session');  // session middleware
      ...
      var usrdt = require('../models/com.utes.mongo.crud');
      const mongostore = require("connect-mongo");
      const conMongo = require("../models/com.utes.mongo.connMongo");
      const samlAssrt = require("./com.utes.saml.user_initiated");
      const fs = require("fs");
	...
	async function getDemoUserSAMLAssert(req, res, next) {
		try{
			if (req.method === 'GET' && req.method !== 'POST') {
				console.log('getDemoUserSAMLAssert:GET:   ' + JSON.stringify(req.body));
			...
			if (req.method === 'POST' && req.method !== 'GET') {
				...
				if (req.body.udata) {
					...
					await samlAssrt.getSamlAssert(user.uid, req, res, next);
