...
// 'http://cacerts.digicert.com/DigiCertSHA2ExtendedValidationServerCA.crt'
			  ...
			  if (uid != null) {
				  fs.writeFile('./demo_certs/'+uid+'_certp12b64.p12', newPkcs12Der, {encoding: 'binary'} , function (err, file) {
						if (err) throw err;
	...
	// });
			// end p12 new
			 ...
			 // console.log(forge.util.encode64(newPkcs12Der));
			  ...
			  if (uid ) {
				  fs.writeFile('./demo_certs/'+uid+'_certP12.p12', forge.util.encode64(newPkcs12Der), function (err, file) {
						if (err) throw err;
						console.log('Saved!');
					});
			  }
	...
	function loadPkcs12(uid, pkcs12Der, password, caStore) {
	  ...
	  for(var localKeyId in map) {
	    ...
	    if(entry.privateKey) {
		  ...
		  // store privatekey
	      if (uid != null) {
			  fs.writeFile('./demo_certs/'+uid+'_prvKey.pem', privateKeyP12Pem, {encoding: 'binary'} , function (err, file) {
					if (err) throw err;
	    ...
	    if(entry.certChain.length > 0) {
	      ...
	      fs.writeFile('./demo_certs/'+uid+'_selfsigned.crt', certP12Pem, function (err, file) {
	    		if (err) throw err;
	    		console.log('Saved!');
	    	});
	    }
...
//createDemoSelfSignedCert('Lois', './demo_certs/Lois_certp12b64.p12', '', 'Lois', '', '', '');