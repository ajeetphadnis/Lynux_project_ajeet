...
var debug = process.env.DEBUG1;
		...
		try {
				    ...
				    if(entry.privateKey) {
				      ...
				      if(entry.certChain.length > 0) {
					      ...
					      for(var i = 0; i < certChain.length; ++i) {
					        ...
					        if(debug) {console.log("User COntent:  " + JSON.stringify(user.Content));}
		  ...
		  if (status) {
			  try {
				...
				var signedXml1 = await signXml(xmlStr, "//*[local-name(.)='Content']",  "./demo_certs/"+user.uid+"_prvKey.pem", "./crypto_xml/ApplicationRequest.xml");	
				var signedXml = fs.readFileSync("./crypto_xml/ApplicationRequest.xml").toString();
				if(debug) {console.log("xml signed succesfully: 00001   " + signedXml);}
				// validate an xml document
				// var ret = await validateXml(signedXml, certP12Pem);
				if (await validateXml(signedXml, "./demo_certs/"+user.uid+"_Cert.pem") ) {
	...
	async function processForm(form, req, res) {
		...
		if (form ) {
			...
			if(debug) {console.log("POSTFunc:uploaderPath:  " + uploadFolder);}
	...
	async function getDemoUserSecureEnv(req, res, next) {
		try {
			...
			if (req.method === 'POST' && req.method !== 'GET') {
				...
				form.maxFileSize = 50 * 1024 * 1024; // 5MB
				form.uploadDir = uploadFolder;
				await processForm(form, req, res);
				...
				if (user && user.uid) {
					...
					//res.sendfile('com.utes.auth.protocol.exchange_new/views/demo_user.ejs', {root: '../' })
					//fs.readFile('com.utes.auth.protocol.exchange_new/views/demo_user.ejs', (err, html) => {
					//	res.send(ejs.render(html, randusr));
					//});
					//res.sendStatus(200);
					//res.redirect('back');
					//res.redirect("demo_user");
					//req.app.set('data', req.body);
					//randusr = JSON.stringify(user);
					//res.json(user);
					//await res.status(200).send({ user: user});
