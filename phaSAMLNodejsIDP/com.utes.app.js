...
function resolveFilePath(filePath) {
  ...
  if (filePath.startsWith('saml-idp/')) {
    // Allows file path options to files included in this package, like
	// config.js
...
function prettyPrintXml(xml, indent) {
  // This works well, because we format the xml before applying the
	// replacements
  ...
  return baseArgv
    ...
    .options({
      ...
      issuer: {
        ...
        required: true,
        alias: 'iss',
        default: 'urn:prathamesh-phadnis.com:idp'
      },
      acsUrl: {
...
function _runServer(argv) {
  ...
  const idpOptions = {
    ...
    responseHandler:        function(response, opts, req, res, next) {
                              ...
                              fs.writeFile("./samlIdpResponse.xml", response, function(err) {
                  				if(err) {
                  			        return console.log(err);
                  			    }
                  		    });
                              console.log("xml Parser1:  " + response); 
                              ...
                              parser.parseString(response, function (err, result) {
                            	  ...
                            	  console.log("xml Parser2:  " + nID);
                            	  var issu = result['samlp:Response']['saml:Issuer'];
                            	  issu = JSON.stringify(issu);
                            	  issu = issu.replace('{\"_\":\"', '');
                            	  issu = issu.replace('","$":{"xmlns:saml":"urn:oasis:names:tc:SAML:2.0:assertion"}}', '');
	                              console.log("xml Parser3:  " + issu);
	                              var auds = result['samlp:Response']['saml:Assertion']['saml:Conditions']['saml:AudienceRestriction']['saml:Audience'];
	                              ...
	                              // auds =
									...
									// '');
	                              console.log("xml Parser4:  " + auds);
                            	  var sub = result['samlp:Response']['saml:Assertion']['saml:Subject'];
                            	  ...
                            	  console.log("xml Parser5:  " + sub);
                            	  var nbfv = result['samlp:Response']['saml:Assertion']['saml:Conditions'].$.NotBefore;
                            	...
                            	// get private key
                            	  ...
                            	  // encode data with the private key using
									// the ES256 algorithm
                            	  const encodedToken = JWT.sign({id: nID, iss: issu, aud: auds, nbf: seconds, iat: seconds}, privateKey, { algorithm: 'ES256', expiresIn: '24h' });
                            	  ...
                            	  var secretKey = fs.readFile('./idp-private-key.pem', 'utf8' , (err, data) => {
                            		  //console.log("pKey:   "  + data);
		                                ...
		                                // sign token
		                                const token = JWT.sign({id: nID, iss: issu, aud: auds, nbf: seconds, iat: seconds}, data, { algorithm: 'RS256',
		                                 expiresIn: '24h' });
    ...
    Object.keys(req.body).forEach(function(key) {
      ...
      if (key === '_authnRequest') {
        ...
        if (req.idp.options.allowRequestAcsUrl && req.authnRequest.acsUrl) {
          ...
          authOptions.destination = req.authnRequest.audience;
          authOptions.forceAuthn = req.authnRequest.forceAuthn;
        }
        res.redirect("http://www.prathamesh-phadnis.com:3000/execServ");
  ...
  app.get(IDP_PATHS.SIGN_IN, function(req, res, next) {
    res.redirect("http://www.prathamesh-phadnis.com:3000/execServ");
  });
