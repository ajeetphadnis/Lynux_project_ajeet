...
function prettyPrintXml(xml, indent) {
	// This works well, because we format the xml before phaApplying the replacements
...
function _runServer(argv) {
	const phaApp = express();
	const httpServer = argv.https ?
		https.createServer({ key: argv.httpsPrivateKey, cert: argv.httpsCert }, phaApp) :
		http.createServer(phaApp);
	const blocks = {};
  ...
  `));
	...
	phaApp.set('host', process.env.HOST || argv.host);
	phaApp.set('port', process.env.PORT || argv.port);
	phaApp.set('views', path.join(__dirname, 'views'));

	/**
	 * View Engine
	 */

	phaApp.set('view engine', 'hbs');

	phaApp.set('view options', { layout: 'layout' })
	phaApp.engine('handlebars', hbs.__express);
	...
	phaApp.use(logger(':date> :method :url - {:referrer} => :status (:response-time ms)', {
		skip: function (req, res) {
			return req.path.startsWith('/bower_components') || req.path.startsWith('/css')
		}
	}));
	phaApp.use(bodyParser.urlencoded({ extended: true }));
	phaApp.use(express.static(path.join(__dirname, 'public')));
	phaApp.use(session({
	...
	phaApp.use(function (req, res, next) {
		if (argv.rollSession) {
	...
	phaApp.get(['/', '/idp', IDP_PATHS.SSO], parseSamlRequest);
	phaApp.post(['/', '/idp', IDP_PATHS.SSO], parseSamlRequest);

	phaApp.get(IDP_PATHS.SLO, parseLogoutRequest);
	phaApp.post(IDP_PATHS.SLO, parseLogoutRequest);

	phaApp.post(IDP_PATHS.SIGN_IN, function (req, res) {
	...
	phaApp.get(IDP_PATHS.METADATA, function (req, res, next) {
		samlp.metadata(req.idp.options)(req, res);
	});

	phaApp.post(IDP_PATHS.METADATA, function (req, res, next) {
	...
	phaApp.get(IDP_PATHS.SIGN_OUT, function (req, res, next) {
		if (req.idp.options.sloUrl) {
	...
	phaApp.get([IDP_PATHS.SETTINGS], function (req, res, next) {
		res.render('settings', {
			idp: req.idp.options
		});
	});

	phaApp.post([IDP_PATHS.SETTINGS], function (req, res, next) {
		Object.keys(req.body).forEach(function (key) {
	...
	phaApp.use(function (err, req, res, next) {
		if (err) {
	...
	console.log(chalk`Starting IdP server on port {cyan ${saml-idp.get('host')}:${saml-idp.get('port')}}...\n`);

	httpServer.listen(phaApp.get('port'), phaApp.get('host'), function () {
	...
	function runAssert(options) {
		const args = processArgs([], options);
		return _runAssert(args.argv);
	}

	function main() {
		const args = processArgs(process.argv.slice(2));
		_runAssert(args.argv);
	}
