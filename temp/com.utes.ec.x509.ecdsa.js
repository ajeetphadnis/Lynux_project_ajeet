...
function crAns1Ecdsakeys() {
  var ECPublicKey = asn1.define("PublicKey", function() {
    this.seq().obj(
...
function crAsn1Eckeys(user, curvType, path, type) {
 // Define ECPrivateKey from RFC 5915
    var ECPrivateKey = asn1.define('ECPrivateKey', function() {
      this.seq().obj(
        this.key('version').int(),
    ...
    if (type === 'private' && curvType === 'prime256v1') {
        ...
        fs.writeFileSync((path+'/'+user), pemKey, 'utf8');
        console.log("privatekey:  " + pemKey);
        return pemKey;
      }
    // Sign data
...
async function crEcdsaKeys(type, format) {
	...
	if (resolve) {
        	...
        	if(!err) {
        	    ...
        	    // pair after encoding
        	    if (type === 'public') {
        		    pubkey = publicKey.toString('utf8');
...
async function crX509CertEcc (type) {
    ...
    await crEcdsaKeys('public', 'pem');
    await crEcdsaKeys('private', 'pem');
    console.log("privatekey:  " + prvkey);
    var sign = await crypto.createSign('sha512'); //sha512
    await sign.update(cert.toString());
    var signature = await sign.sign(prvkey, 'base64');
...
async function getECPubkey(type) {
    ...
    function createECKeys(user, curvType, path, type) {
      /*
      ...
      */
       console.log("curveType:   " + curvType);
       var wsRegex = /^\s*$(?:\r\n?|\n)/gm; 
      const keyPair = crypto.createECDH(curvType); // 1. prime256v1, 2. secp256k1
        ...
        if (type === 'private' && curvType === 'prime256v1') {
          ...
          pkey =  pkey.replace(/(\r\n|\n|\r)/gm, "");   
          fs.writeFileSync((path+'/'+user), pkey, 'utf8');
        ...
        if (type === 'private' && curvType === 'secp256k1') {
        ...
        if (type === 'public' && curvType === 'secp256k1') {
 ...
 async function getCert(type, format) {
        try {
            ...
            if(type === 'public' || type === 'private') { 
              var ret = await crEcdsaKeys(type, format); //.then(pubkey => { console.log( pubkey )});
              console.log("getCert002:  " + ret);
              return ret;
            }
    	      //ret = await crEcdsaKeys(type, format); //.then(prvkey => { console.log( prvkey )});
    	      //console.log("getCert003: " + ret);
    	      //await crX509CertEcc(type);
            if (type === 'asn1') {
    	        var ans1key = crAns1Eckeys();
              return ans1key;
            }
            if (type === 'cert') {
              await crX509CertEcc(type);
...
exports.crAns1Eckeys = crAsn1Eckeys;
exports.crAns1Ecdsakeys = crAns1Ecdsakeys;
exports.createECDHPrvKey = createECDHPrvKey;
exports.createECKeys = createECKeys;
...
//crAsn1Eckeys(); // works
//createECDHPrvKey(); // works
//crAns1Ecdsakeys();
//crX509CertEcc();
///crAsn1Eckeys('jyotsna', 'prime256v1', 'jwks/ecdsa_certs/', 'private');
  // Define ECPrivateKey
//var key = createECKeys('jyotsna', 'prime256v1', 'jwks/ecdsa_certs/', 'private');  // 1. prime256v1, 2. secp256k1
//console.log("EC Key:  " + key);
