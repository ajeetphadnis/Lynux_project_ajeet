...
var crypto = require("crypto");
    ...
    async function crX509CertEcdsa() {
        console.log('Generating 2048-bit key-pair...');
        var keys = forge.pki.rsa.generateKeyPair(2048);
        console.log('Key-pair created.');
	var prvk = await ecdsa.crEcdsaKeys('private', 'pem');
	var pubk = await ecdsa.crEcdsaKeys('public', 'der');
	//var ans1key = await ecdsa.crAns1Ecdsakeys();
  var prvKey = ecdsa.createECDHPrvKey();
	console.log('prvkey:  ' + prvk);
	console.log('pubkey:  ' + pubk);
	console.log('prvKdy:  ' + prvKey);
        ...
        // read : https://security.stackexchange.com/questions/147803/how-are-ecdsa-signatures-computed-for-x509-certificates
        console.log('signingkey:  '+ cert);
        //console.log('signingkey:  '+ JSON.stringify(JSON.parse(JSON.stringify(cert))));
        // Sign data
        var sign = crypto.createSign('sha512');
        sign.update(JSON.stringify(JSON.parse(JSON.stringify(cert))));
        var signature = sign.sign(prvk, 'utf8').toString('base64');
        //sign.toString('base64');
        //cert.sign(prvk, forge.md.sha256.create());
        ...
        var pem = {
          privateKey: prvk,
          publicKey: pubk,
          certificate: forge.pki.certificateToPem(signature)
        };
