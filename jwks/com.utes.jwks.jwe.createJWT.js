...
async function getX509Details (uid, x5cStr, pass) {
	...
	if(debug) {console.log('getX509Details:001   ' + pem);}
	...
	if(debug) {console.log("getX509Details007:  " + JSON.stringify(payload));}
...
const getCA_P12_PrivKey = async ( uid, filePath, pass ) => {
    ...
    if(debug) {console.log("loadCA_P12Cert001: " + filePath +  '  ' + pass);}
    var p12 = fs.readFileSync(filePath);
    console.log("loadCA_P12Cert002:priv: "+ filePath +  '  ' + pass);
...
var store = JWK.createKeyStore();
	async function creJWTJWE (uid, path, pass, req, res, next) {
		...
		if(debug) {console.log("pubkey:  " + JSON.stringify(pubKey));}
		...
		if(debug) {console.log("SecretKey:   " + JSON.stringify(skey));}
		var options = 
		{
		    zip: false,
		    compact: true,
	...
	async function creDemoJWTJWE (uid, path, pass) {
		var store = JWK.createKeyStore();
		await store.generate("RSA",2048,{alg:"RS256", key_ops:["sign", "decrypt", "unwrap"]});
		lkey = (await store.get());
		JSON.stringify(lkey.toJSON(true));
		...
		token  = await JWE.createEncrypt(options, skey).update(token, "utf8").final();
		if(debug) {console.log(token);}
		await fs.writeFileSync('./demo_jwtokens/'+uid+"_jwetok.jwt", token, null, '  ');
	}
	...
	async function creJWTJWS (uid, path, pass, req, res, next) {
		...
		var dt = new Date();
		var exp = new Date(dt.getTime() + (20 * 60 * 1000));
		let message = await getX509Details(uid, x5cChain, pass);
	    ...
	    if(debug) {console.log(tokMain);} //.startsWith('protected:' ));
	    await fs.writeFileSync('./user_jwtokens/'+uid+"_x5jwtok.jwt", tokMain, null, '  ');
	    return tokMain;
	}
	...
	async function creDemoJWTJWS (uid, path, pass) {
	    ...
	    var tokMain = head1+'.'+payld+'.'+head;
	    if(debug) {console.log(tokMain); }//.startsWith('protected:' ));
	    fs.writeFileSync('./demo_jwtokens/'+uid+"_x5jwtok.jwt", tokMain, null, '  ');
	    return tokMain;
	}
		 ...
		 try{
				...
				if (req.method === 'POST' && req.method !== 'GET') {
					...
					if (req.body.udata) {
						...
						console.log("getDemoUserJWT001:   called ...." + user.uid);
						...
						user.newuser.orgName = 'utesdemo.com';
						 ...
						 uid = user.uid;
						 console.log("getDemoUserJWT:POST:  " + uid);
						 await creDemoJWTJWS (uid, './demo_jwtokens/', user.pass);
				 		 await creDemoJWTJWE(uid,'./demo_jwtokens/', user.pass);
					 	//ksVals.CustomerId =  req.app.session.clnt;
				 		var x5jwtok = fs.readFileSync('./demo_jwtokens/'+uid+'_x5jwtok.jwt').toString();
				 		var jwtok = fs.readFileSync('./demo_jwtokens/'+uid+'_jwetok.jwt').toString();
				 		user.x5jwtok = x5jwtok;
				 		user.jwtok = jwtok;
	...
	exports.creDemoJWTJWS = creDemoJWTJWS;
	exports.creDemoJWTJWE = creDemoJWTJWE;
	exports.getDemoUserJWT = getDemoUserJWT;
	...
	//creJWTJWS ('karan123456', './JWKSets/', 'password', '', '', '');
